...
### Implement Gated Linear Unit and Gated Residual Network
...
class TemporalLayer(nn.Module): ## Do we really need this - as linear now accepts 3D data and that would have the same functionality!? Look into this later.
    ...
    def forward(self, embedding, context=None):
        ...
        transformed_embeddings = torch.stack(
            [self.transformed_inputs[i](embedding[
                Ellipsis, i*self.input_size:(i+1)*self.input_size]) for i in range(self.output_size)], axis=-1) ## Ellipsis 
